...
func (k msgServer) Sponse(goCtx context.Context, msg *types.MsgSponse) (*types.MsgSponseResponse, error) {
	...
	change := types.NewDefaultStreamRecordChangeWithAddr(msg.Creator).WithRateChange(msg.Rate.Neg())
	...
	change = types.NewDefaultStreamRecordChangeWithAddr(msg.To).WithRateChange(msg.Rate)
