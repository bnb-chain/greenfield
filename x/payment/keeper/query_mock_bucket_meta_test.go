...
func TestMockBucketMetaQuerySingle(t *testing.T) {
	keeper, ctx := keepertest.PaymentKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	msgs := createNMockBucketMeta(keeper, ctx, 2)
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			response, err := keeper.MockBucketMeta(wctx, tc.request)
			if tc.err != nil {
...
func TestMockBucketMetaQueryPaginated(t *testing.T) {
	keeper, ctx := keepertest.PaymentKeeper(t)
	wctx := sdk.WrapSDKContext(ctx)
	msgs := createNMockBucketMeta(keeper, ctx, 5)

	request := func(next []byte, offset, limit uint64, total bool) *types.QueryAllMockBucketMetaRequest {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.MockBucketMetaAll(wctx, request(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.MockBucketMetaAll(wctx, request(next, 0, uint64(step), false))
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.MockBucketMeta), step)
			require.Subset(t,
				nullify.Fill(msgs),
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := keeper.MockBucketMetaAll(wctx, request(nil, 0, 0, true))
		require.NoError(t, err)
	...
	t.Run("InvalidRequest", func(t *testing.T) {
		_, err := keeper.MockBucketMetaAll(wctx, nil)
