...
func networkWithMockObjectInfoObjects(t *testing.T, n int) (*network.Network, []types.MockObjectInfo) {
	t.Helper()
	...
	return network.New(t, cfg), state.MockObjectInfoList
}

func TestShowMockObjectInfo(t *testing.T) {
	net, objs := networkWithMockObjectInfoObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetMockObjectInfoResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.MockObjectInfo)
				require.Equal(t,
					nullify.Fill(&tc.obj),
...
func TestListMockObjectInfo(t *testing.T) {
	net, objs := networkWithMockObjectInfoObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListMockObjectInfo(), args)
			require.NoError(t, err)
			var resp types.QueryAllMockObjectInfoResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
