...
function init() {
    ...
    for ((i=0;i<${sp_size};i++));do
      ...
      ${bin} keys add sp${i} --keyring-backend test --home ${workspace}/.local/validator0 > ${workspace}/.local/validator0/sp${i}_info
      ${bin} keys add sp${i}_fund --keyring-backend test --home ${workspace}/.local/validator0 > ${workspace}/.local/validator0/sp${i}_fund_info
    done
}
...
function generate_genesis() {
    ...
    for ((i=0;i<${sp_size};i++));do
      #create sp and sp fund account
      sp_addrs=("$(${bin} keys show sp${i} -a --keyring-backend test --home ${workspace}/.local/validator0)")
      spfund_addrs=("$(${bin} keys show sp${i}_fund -a --keyring-backend test --home ${workspace}/.local/validator0)")
