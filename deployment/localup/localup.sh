...
function start() {
    ...
    for ((i=0;i<${size};i++));do
        ...
        nohup ${bin} start --home ${workspace}/.local/validator${i} \
            --address tcp://0.0.0.0:$((${VALIDATOR_ADDRESS_PORT_START}+${i})) \
            --grpc.address 127.0.0.1:$((${VALIDATOR_GRPC_PORT_START}+${i})) \
            --grpc-web.address localhost:$((${VALIDATOR_GRPC_WEB_PORT_START}+${i})) \
            ...
            --api.address tcp://localhost:$((${VALIDATOR_COSMOS_PORT_START}+${i})) \
            --api.enable \
